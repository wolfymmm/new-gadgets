...
export const ProductPage = () => {
  ...
  const isLiked = favorites.some(
    item => item.id === productId || (item as any).itemId === productId,
  );
  ...
  const findTargetVariant = (targetColor: string, targetCapacity: string) => {
    ...
    const baseId = normalize(
      (product as any).namespaceId || getModelBase(productId || ''),
    );
    ...
    return allProducts.find(p => {
      ...
      const isSameCapacity =
        normalize(p.capacity) === normalize(targetCapacity);
  ...
  const handleLikeClick = () => {
    ...
    if (isLiked) {
      ...
      const catalogItem =
        allProducts.find(p => p.itemId === productId) || (product as any);
  ...
  const handleAppSuggestCarousel = () => {
    // Тримаємо логіку чистішою
  };
                  ...
                  return (
                    <label key={color} className={s.colorLabel}>
                      <input
                        ...
                        onChange={() => {
                          if (variant) {
                            navigate(`/${category}/${variant.itemId}`);
                          }
                        }}
                      />
                      <span
                        ...
                        onClick={() =>
                          variant && navigate(`/${category}/${variant.itemId}`)
                        }
                      />
                    </label>
                  );
            ...
            <div className={s.productCapacity}>
              ...
              <div className={s.capacityButtons}>
                {product.capacityAvailable.map(capacity => {
                  ...
                  if (!variant) {
                    const baseId =
                      (product as any).namespaceId ||
                      getModelBase(productId || '');

                    variant =
                      allProducts.find(
                        p =>
                          ((p as any).namespaceId === baseId ||
                            getModelBase(p.itemId) === baseId) &&
                  ...
                  return (
                    <label key={capacity} className={s.capacityLabel}>
                      <input
                        ...
                        className={s.visuallyHidden}
                        checked={
                          normalize(capacity) === normalize(product.capacity)
                        }
                        onChange={() => {
                          if (variant) {
                            navigate(`/${category}/${variant.itemId}`);
        ...
        <div className={s.containerProducts}>
          ...
          <div className={s.carouselButtons}>
            ...
            <button
              className={s.carouselArrow}
              onClick={handleNext}
              disabled={startIndex + 4 >= suggestedProducts.length}
            >
